...
static MprSize readSocket(MprSocket *sp, void *buf, MprSize bufsize);
static MprSize writeSocket(MprSocket *sp, void *buf, MprSize bufsize);
 ...
 */
MprSize mprReadSocket(MprSocket *sp, void *buf, MprSize bufsize)
{
    mprAssert(sp);
    mprAssert(buf);
    mprAssert(bufsize > 0);
 ...
 */
MprSize mprWriteSocket(MprSocket *sp, void *buf, MprSize bufsize)
{
    mprAssert(sp);
    mprAssert(buf);
    mprAssert(bufsize > 0);
 ...
 */
MprSize mprWriteSocketString(MprSocket *sp, cchar *str)
{
    return mprWriteSocket(sp, (void*) str, strlen(str));
}


MprSize mprWriteSocketVector(MprSocket *sp, MprIOVec *iovec, int count)
{
    MprSize     total, len, i, written;
    char        *start;
...
#if !LINUX || __UCLIBC__
static MprSize localSendfile(MprSocket *sp, MprFile *file, MprOffset offset, MprSize len)
{
 ...
 */
MprSize mprSendFileToSocket(MprSocket *sock, MprFile *file, MprOffset offset, int bytes, MprIOVec *beforeVec, 
...
int mprGetSocketInfo(cchar *ip, int port, int *family, int *protocol, struct sockaddr **addr, socklen_t *addrlen)
{
    ...
    if ((hostent = getipnodebyname(ip, AF_INET, 0, &err)) == NULL) {
        ...
        memcpy((char*) &sa6->sin6_addr, (char*) hostent->h_addr_list[0], (MprSize) hostent->h_length);
        ...
        *addr = (struct sockaddr*) sa6;
        ...
        memcpy((char*) &sa->sin_addr, (char*) hostent->h_addr_list[0], (MprSize) hostent->h_length);
