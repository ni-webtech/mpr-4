...
MprChar *mcontains(MprChar *str, cchar *pattern, MprSize limit)
{
    MprChar     *cp, *s1;
    cchar       *s2;
    MprSize      lim;
 ...
 */
MprSize mcopy(MprChar *dest, cchar *src)
{
    MprSize      len;
 ...
 */
int mncasecmp(MprChar *s1, cchar *s2, MprSize n)
{
    int     rc;
...
int mncmp(MprChar *s1, cchar *s2, MprSize n)
{
    mprAssert(0 <= n && n < MAXSIZE);
...
MprSize mncopy(MprChar *dest, MprSize destMax, cchar *src, MprSize len)
{
...
MprSize mspn(MprChar *str, cchar *set)
{
    cchar   *sp;
    int     count;
