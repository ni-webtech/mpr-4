...
static MprSize fillBuf(MprFile *file);
...
static char *findNewline(cchar *str, cchar *newline, MprSize len, MprSize *nlen)
{
    char    *start, *best;
    int     i, newlines;
...
/*
    ...
    Return NULL on eof.
 */
char *mprGets(MprFile *file, MprSize maxline, MprSize *lenp)
{
    MprBuf          *bp;
    MprFileSystem   *fs;
    MprSize          size, len, nlen, consumed;
 ...
 */
MprSize mprPuts(MprFile *file, cchar *str)
{
    MprBuf  *bp;
    MprSize  total, bytes, count;
    char    *buf;
 ...
 */
MprSize mprPutc(MprFile *file, int c)
{
    mprAssert(file);
...
MprSize mprRead(MprFile *file, void *buf, MprSize size)
{
    MprFileSystem   *fs;
    MprBuf          *bp;
    MprSize          bytes, totalRead;
    void            *bufStart;
...
MprSize mprWrite(MprFile *file, cvoid *buf, MprSize count)
{
    MprFileSystem   *fs;
    MprBuf          *bp;
    MprSize          bytes, written;
...
MprSize mprWriteString(MprFile *file, cchar *str)
{
    return mprWrite(file, str, strlen(str));
}


MprSize mprWriteFormat(MprFile *file, cchar *fmt, ...)
{
    va_list     ap;
    char        *buf;
    MprSize     rc;
