...
class Bit 
{
    ...
    function findComponents() {
        let dir = Path(cmd.options.init)
dump("REQ", spec.required)
dump("OPT", spec.optional)
...
print("FIND " + component)
            ...
            if (path.exists) {
                try {
                    ...
                    //MOB spec.components[component].path = 
                    load(path)
                } catch (e) {
                    App.log.debug(2, e)
    ...
    public function probe(file: Path, search = null, options = {}): Path {
        let path: Path
        if (file.exists) {
            path = file
        } else {
...
public function probe(file: Path, search = null, options = {}): Path {
    return b.probe(file, search, options)
}
/*
    @copy   default
