...
class Bit 
{
    ...
    function initialize() {
        ...
        let bbit = dir.join('build.bit')
        if (bbit.exists && !cmd.options.overwrite) {
            throw 'The ' + bbit + ' file already exists. Use bit --overwrite'
        }
        bbit.write('/*\n    build.bit -- Master Build It File\n\n    Generated by bit.\n */\n\nbit(' + 
            serialize(template, {pretty: true}) + ')\n')
    }

    function findComponents() {
        let dir = Path(cmd.options.init)
print("DIR " + dir)
        for each (f in spec.required + spec.optional) {
            let path = dir.join('bit/components', f + '.bit')
print("FIND " + path)
            if (path.exists) {
print("LOAD COMP " + path)
                try {
                    load(path)
                } catch (e) {
                    let kind = spec.required.contains(f) ? 'Required' : 'Optional'
                    whyMissing(kind + ' ' + e)
                }
            } else {
    ...
    function probe(file: Path, search = null, options = {}): Path {
        let path: Path
        if (file.exists) {
            path = file
        } else {
    ...
    public function loadSpec(o) {
        ...
        for each (path in toBlend.blend) {
            currentBit = currentBit.dirname.join(path)
            load(currentBit)
            currentBit = saveCurrent
        }
        debug('Loading ' + currentBit)
    ...
    function prepBuild(cross) {
        ...
        if (cmd.options.debug) {
            App.log.activity('Debug', 'Bit Configuration: \n\nBit = ' + serialize(spec, {pretty: true}))
        }
    }
    ...
    function getOrderedTargets() {
        ...
        if (cmd.options.verbose) {
            ...
            App.log.activity('Targets', names)
        }
    }
...
dump('FAILED', target)
        ...
        App.log.activity('Link', target.name)
        App.log.debug(2, command)
...
dump('FAILED', target)
    ...
    function setTokens(cross: Boolean) {
        ...
        if (cmd.options.verbose) {
            App.log.activity('Building', spec.settings.title + ' for ' + spec.OS + ', ' + spec.ARCH);
        }
    }
    ...
    function missing(component) {
        //  MOB - need to check mandatory and optional
        App.log.activity('Init', 'Component ' + component + ' not included')
    }
}
